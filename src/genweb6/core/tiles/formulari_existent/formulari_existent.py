...
validator.WidgetValidatorDiscriminators(
    ...
    field=IFormulariExistent['content_uid']
)


class FormulariExistent(Tile):
    ...
    def item_macros(self):
        ...
        if default_view and IBrowserView.providedBy(default_view):
            ...
            template = queryMultiAdapter((default_view, default_view.request), IPageTemplate, default=None)
            if template:
                return BoundPageTemplate(template, default_view.context).macros
            else:  # es contingut_upc
                return BoundPageTemplate(default_view.template, default_view.context).macros
        elif default_view:
            # FSPageTemplate
